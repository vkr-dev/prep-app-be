...
- **System Design** - 16 subtopics (158 questions): the original 9 fundamentals (scalability, networking, database design/scaling, caching, CAP theorem/consistency, distributed systems, reliability/fault tolerance, security at scale, observability at scale), the sequential "Designing an AI-Powered System, Step by Step" walkthrough, plus 6 fully worked system design **case studies**, each a complete design narrated via Situation-Task-Action-Result (used here purely as a case-study storytelling structure, explicitly noted in the content itself as distinct from STAR's actual role in behavioral interviews and distinct from the real system-design framework covered in the sequential-walkthrough subtopic above). Each case study's practice questions open with a framing question ("what system are we designing, with what capabilities") and then heavily detailed, adversarial self-critique questions spanning every angle of the design, each tagged with a bracketed label naming exactly what it's grilling (e.g. `[Loan Limit Consistency]`, `[CAP Tradeoff]`, `[Prompt Refinement]`) so the specific concern being tested is always clear at a glance:
  - *General*: "Design a Buy-Now-Pay-Later Payment Platform", "Design a Ride-Sharing Dispatch System"
  - *AI*: "Design an AI-Powered Code Review Agent", "Design an AI-Powered Real-Time Fraud Detection System" (deliberately cross-references the BNPL case study, since both plug into the same hypothetical payments business)
  - *AI-assisted-interview format*: "Design a Windows Notification System" - models an interview where the candidate is expected to use an AI assistant as a design collaborator and is evaluated on how prompts were framed, refined after a weak first answer, and critically evaluated rather than accepted - the case study's Action section narrates the actual prompting/refinement/critique sequence, and its questions include prompting-technique and AI-output-evaluation angles alongside standard system design grilling.
  - *Design-plus-coding format*: "Design a Proximity Search System for Food Delivery" (geohashing, the boundary/precision tradeoffs, read-heavy vs. write-heavy indexing) - models an interview with multiple design follow-ups ending in an actual coding exercise; its final practice question is a real coding problem (implementing Haversine-based radius filtering) with a working Python solution as the answer, not just a discussion question.
...
In total: 85 subtopics, 710 real Q&A pairs, and 85 reading passages across 7 curated topics.
