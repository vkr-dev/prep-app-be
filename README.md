...
Subtopic coverage is deliberately broad per topic, not just a handful of headline areas - the goal is that a topic's page is genuinely comprehensive, not a sampler. Currently: Java has 14 subtopics (112 questions) spanning core OOP, collections, concurrency, exceptions, streams, memory/JVM, generics, I/O & NIO, serialization, design patterns, annotations & reflection, modern language features (records/sealed classes/pattern matching), enums & nested classes, date/time & regex, and JDBC. Spring Boot has 12 subtopics (96 questions) adding AOP, caching, async/scheduling/events, Actuator/observability, and microservices/Spring Cloud on top of the original core/DI/MVC/JPA/security/config/testing set. RAG and LangChain each have 10 subtopics (80 questions each), adding advanced patterns (Self-RAG/CRAG/GraphRAG), prompt engineering, production scaling/cost, and security/privacy for RAG; and LangGraph/multi-agent orchestration, output parsing, tracing/LangSmith, and deployment for LangChain. In total: 46 subtopics, 368 real Q&A pairs, and 46 reading passages across the 4 curated topics.
