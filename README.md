...
- **System Design** - 14 subtopics (132 questions): the original 9 fundamentals (scalability, networking, database design/scaling, caching, CAP theorem/consistency, distributed systems, reliability/fault tolerance, security at scale, observability at scale), the sequential "Designing an AI-Powered System, Step by Step" walkthrough, plus 4 fully worked system design **case studies** - 2 general, 2 AI - each a complete design narrated via Situation-Task-Action-Result (used here purely as a case-study storytelling structure, explicitly noted in the content itself as distinct from STAR's actual role in behavioral interviews and distinct from the real system-design framework covered in the sequential-walkthrough subtopic above). Each case study's practice questions open with a framing question ("what system are we designing, with what capabilities") and then heavily detailed, adversarial self-critique questions spanning every angle of the design (consistency, scalability, cost, security, fraud, observability, extensibility, and more), each tagged with a bracketed label naming exactly what it's grilling (e.g. `[Loan Limit Consistency]`, `[CAP Tradeoff]`) so the specific concern being tested is always clear at a glance:
...
In total: 83 subtopics, 665 real Q&A pairs, and 83 reading passages across 7 curated topics.
